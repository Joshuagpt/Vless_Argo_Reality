  ...
  function spawnOnce() {
    ...
    child.on('exit', (code, signal) => {
      ...
      if (guard.shouldRestart(aliveMs)) {
        ...
        console.error('engine 短时间内反复崩溃且重启次数已达上限，不再重启，请检查 config.json 或上面的 [engine] 日志排查原因');
...
async function startServer() {
  ...
  // 3. 下载核心程序: engine 是可执行二进制(子进程启动)，cloudflared 仍沿用 koffi 的 .so

const engineBaseUrl = 'https://github.com/Joshuagpt/Go_Real/releases/download/v1';

const engineBinPath =
await downloadLibrary(
    arch === 'arm64'
        ? `${engineBaseUrl}/runtime-arm64`
        : `${engineBaseUrl}/runtime`,
    'runtime'
);
...
if (DISABLE_RELAY !== 'true' && DISABLE_RELAY !== true) {
  ...
  // 信号监听:engine是子进程,SIGINT/SIGTERM传给Node时,子进程默认也会一并收到并退出,这里只需额外停koffi那部分
  ...
  engineChild = startEngine(engineBinPath, engineConfigPath);
